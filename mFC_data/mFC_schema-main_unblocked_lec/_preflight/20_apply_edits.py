#!/usr/bin/env python3
"""Build `mFC_schema-main_unblocked_lec/` from the deposit by applying the declared edits.
...
UNBLOCKED = os.path.join(ROOT, 'mFC_schema-main_unblocked_lec')
LOGDIR = os.path.join(ROOT, 'lec_replication_run', 'logs')

IN_MIRROR = os.path.join(ROOT, 'lec_replication_run', 'data', 'Intermediate_objects') + '/'
OUT_EPHYS = os.path.join(ROOT, 'lec_replication_run', 'Output_folder', 'ephys') + '/'
OUT_BEHAV = os.path.join(ROOT, 'lec_replication_run', 'Output_folder', 'behaviour') + '/'
...
EDITS: dict[str, list] = {
    ...
    'Figure2.ipynb': [
        *paths('F2-01', 1, 2, 3, OUT_EPHYS, 'Figure2'),
        ...
        # ---- LEC port: the two edits below exist ONLY in mFC_schema-main_unblocked_lec.
        # Neither changes a computed quantity; both are cohort/coverage properties of
        # running the deposit on a dataset it never saw.
        ins('LEC-01', 'COVERAGE', 7, 4, "'ab03':1,'ah07':2}",
            ["Mice_maze_dic.update({'ah08':1,'ah10':1,'ly05':1,'ly06':1,'ly07':1})   # UNBLOCK-LEC-01"],
            "the LEC mice (ah08 ah10 ly05 ly06 ly07) are not in the hardcoded mouse->maze "
            "table, and cell 16 does Mice_maze_dic[mouse] inside a bare try: the KeyError "
            "prints 'speed_dic not made' per session, speed_dic stays empty, cell 18's "
            "distances=speed_dic[rd][ses] autovivifies, its except swallows the TypeError, "
            "and ALL SIX phase/state/time dicts stay silently empty -- nothing downstream is "
            "computed (runner gate 1 is what catches it). Mapped to maze 1 because cell 14 "
            "loads only mazes '1' and '2'; the LEC Maze1/Maze2_measurements.txt written by "
            "15_export_lec_to_flat.py encode pixels_per_cm == 1, since LEC XY_raw is already "
            "in cm (frame_rate=60 is left as deposited)"),
        ins('LEC-04', 'COVERAGE', 7, 2, "'ab03':6,'ah07':6}",
            ["Mice_cohort_dic.update({'ah08':7,'ah10':7,'ly05':7,'ly06':7,'ly07':7})   # UNBLOCK-LEC-04"],
            "the second hardcoded mouse table. Cell 61 does cohort=Mice_cohort_dic[mouse] for "
            "every 3_task_all recday, outside any try, and never reads `cohort` again. With "
            "the LEC single-day cohort populated (attempt 2) the lookup raised KeyError 'ah08' "
            "and aborted Figure2 after 4.9 h; in attempt 1 the loop body never ran because the "
            "cohort was empty, which is why LEC-01 covered only the maze table. Cohort number 7 "
            "is inert: Cohort_ephys_type_dic and the Mice/Ephys_mice arrays derived from this "
            "dict are defined in cell 7 and consumed nowhere. Placed after the dict literal so "
            "those derived arrays are unchanged"),
        ins_before('LEC-02', 'COVERAGE', 61, 40,
                   'percentile_neuron_bool_all=np.vstack((percentile_neuron_bool_all))',
                   ["    if len(percentile_neuron_bool_all)==0:   # UNBLOCK-LEC-02",
                    "        print('no 3_task_all recdays -- cohort is empty on LEC')   # UNBLOCK-LEC-02",
                    "        continue   # UNBLOCK-LEC-02"],
                   "cell 61 stacks per-recday booleans over the 3_task_all cohort with "
                   "np.vstack OUTSIDE any try. LEC has no single-day recdays, so the list is "
                   "empty and np.vstack([]) raises ValueError, aborting the notebook. The "
                   "cell is plot-only (pie charts + proportion prints), no later cell reads "
                   "a name it defines, and the guard sits after line 1's use_both=False so "
                   "the kernel state downstream matches the PFC run exactly"),
    ],
    ...
    'Figure5_Figure6.ipynb': [
        *paths('F6-01', 2, 2, 3, OUT_EPHYS, 'Figure5_Figure6 -- was a Windows path'),
        ...
        # ---- LEC port (this copy only). Same class as LEC-01/02: the deposit only works for
        # the session layout it was written against.
        ins_before('LEC-03', 'COVERAGE', 17, 173, "exec('ephys_=ephys_ses_'+str(session)+'_')",
                   ["                    if session in ses_not_found:   # UNBLOCK-LEC-03",
                    "                        continue   # UNBLOCK-LEC-03"],
                   "cell 17 loads each session's Neuron_ file into a name built with exec "
                   "(ephys_ses_<s>_), appending missing sessions to ses_not_found. Its FIRST "
                   "consumer loop (line 70) skips ses_not_found before the reverse exec; its "
                   "SECOND (line 173) does not. A session index that no earlier recday ever "
                   "defined therefore raises NameError -- on LEC, ah08_20250624_20250625 has 11 "
                   "sessions of which 9 and 10 are zero-trial, and no recday before it has a "
                   "session 9, so the run died there after 21947 s. On PFC (max 9 sessions) the "
                   "name always happened to exist from an earlier recday, holding THAT recday's "
                   "neurons; the stale value is then discarded because the Location_ load two "
                   "lines later fails and continues, so PFC results are unaffected. This guard "
                   "makes the second loop skip exactly the sessions the first loop skips; it "
