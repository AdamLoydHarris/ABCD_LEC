#!/usr/bin/env python
"""
Task-phase ring sweep (trial-averaged) for PFC — sibling of code/run_taskphase_ring.py.
...
def load_data_dic(args, recdays=None):
    """PFC: build on the fly with compute_norm=True (needed for Neurons_norm).

    Built per-recday by the caller to bound memory (~4 GB across all 25).
    """
    return glm.build_data_dic_from_pfc(_resolve(args.data_folder),
                                       recdays, verbose=False, compute_norm=True)


def pfc_recdays(args):
    import numpy as _np
    return [str(x) for x in _np.load(
        _resolve("mFC_data/data/MetaData/combined_ABCDonly_days.npy"),
        allow_pickle=True)]
...
def run(args, data_dic=None, sleep_loader=None, region="LEC"):
    ...
    recdays = args.recdays or pfc_recdays(args)
    n_done = 0
    for rd in recdays:
        data_dic = load_data_dic(args, [rd])   # per-recday: bounds memory
        if rd not in data_dic:
            print(f"{rd}: no usable sessions, skipping", flush=True)
            continue
...
if __name__ == "__main__":
    ...
    loader = None
    if a.project:
        loader = lambda rd: ph.load_pfc_sleep_files(rd, _resolve(a.data_folder))  # noqa: E731
    run(a, sleep_loader=loader, region="PFC")
