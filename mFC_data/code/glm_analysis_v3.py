"""
GLM analysis — V3, PFC copy.

Seeded from `mFC_data/code/glm_analysis_v2.py` (left untouched) and carrying exactly the V3
hunks of `code/glm_analysis_v3.py` -- `check_mirror_parity.py` asserts that every function
the two V3 copies share has identical source. The one PFC-specific addition, inherited from
the v2 copy, is `build_data_dic_from_pfc(data_folder, recdays)` at module top: it builds the
nested-dict shape `run_glm_analysis` consumes from the PFC directory layout (Neuron_raw /
Location_raw / XY_raw / trialtimes / Task_data .npy under mFC_data/data/). PFC has no head
tracking and no poke tables, so pass `regressors_to_include` without `head_direction` and the
pokes -- which the V3 reduced designs never contain, so they are fitted identically here and
in LEC.

What V3 changes (see `GLM_V3.md`)
---------------------------------
1. `compute_decile_edges(..., fixed_range=(lo, hi))`: equal-width edges over an explicit range
   in the variable's own units, ignoring the data -- so a bin means the same seconds in every
   recday and both datasets, which quantile placement cannot give.
2. `run_glm_analysis(..., binning_overrides={regressor: {'scheme', 'range_s', 'n_bins'}})`:
   per-regressor bin placement for the continuous regressors, on top of the global
   `continuous_binning`. A `'uniform'` override with `range_s` (seconds) uses (1); rows whose
   value falls outside the range -- 250 ms windows straddling a reward, whose block MEAN was
   pulled up by the previous leg's tail -- are NaN-ed (coded as the first bin under reference
   coding, where a leg-start row belongs) and counted.
3. `_resolve_regressor_groups(..., n_cols_override=)` and `compute_tuning_arrays(...,
   n_cols_override=)`: a non-default bin count for an overridden regressor keeps the column map
   consistent everywhere.
4. `CV_results[recday]['bin_occupancy']`: rows per bin, edges (in seconds for the time
   regressors), legs reaching each bin, and the out-of-range count, for every overridden
   regressor; also printed in the log. `CV_results[recday]['engine'] = GLM_VERSION`.
...
#: Engine version, stamped into every cross-validated result as `CV_results[recday]['engine']`.
GLM_VERSION = 'v3'
...
def _extract_head_direction(hd_raw, n_expected):
    ...
    Inert for this dataset -- PFC has `HD_raw=None` -- but mirrored from the LEC copy,
    which is the repo convention: the two `glm_analysis_v2.py` files must not diverge.
...
def compute_decile_edges(values, n_bins=10, outlier_pct=1, scheme='decile', fixed_range=None):
    ...
    `fixed_range=(lo, hi)` (V3) places `n_bins` equal-width edges over exactly `[lo, hi]`, in
    the units of `values`, IGNORING the data -- so the edges are identical in every recday and
    in both datasets, which quantile placement cannot give and data-range uniform placement
    does not give either (the 1-99 % trimmed range differs per recday). Only valid with
    `scheme='uniform'`. The outer edges are still -inf/+inf so `apply_onehot` never drops a
    finite value; a caller that wants out-of-range values excluded masks them first
    (`run_glm_analysis` does, and counts them).
    """
    if fixed_range is not None:
        if scheme != 'uniform':
            raise ValueError(f"fixed_range requires scheme='uniform', got {scheme!r}")
        lo, hi = (float(fixed_range[0]), float(fixed_range[1]))
        if not hi > lo:
            raise ValueError(f"fixed_range must satisfy lo < hi, got {fixed_range!r}")
...
def _resolve_regressor_groups(regressors_to_include, gp_n_bins=10,
                              parameterization='all_bins', n_cols_override=None):
    ...
    `n_cols_override` (V3): `{regressor: n_bins}` for regressors coded with a non-default bin
    count (the `binning_overrides` of `run_glm_analysis`). Applied BEFORE the reference-coding
    decrement, so a 6-bin block becomes 5 columns under 'reference_coded' like every other
    block. Pass the same mapping to `compute_tuning_arrays`.
    """
    ...
    if n_cols_override:
        for k, v in n_cols_override.items():
            k = _REGRESSOR_NAME_ALIASES.get(k, k)
            if k not in n_cols_per:
                raise ValueError(f"n_cols_override names unknown regressor {k!r}")
            n_cols_per[k] = int(v)
...
def run_glm_analysis(mouse_recdays, data_dic,
    ...
    binning_overrides : dict or None (V3)
        Per-regressor bin placement, overriding `continuous_binning` for the named
        continuous regressors (speed, acceleration, time_*, distance_*):
        `{'time_from_reward': {'scheme': 'uniform', 'range_s': (0.0, 30.0), 'n_bins': 10}}`.
        `scheme='uniform'` with `range_s` gives equal-width bins over that many SECONDS
        (converted to raw-bin units with `bin_size_ms`) -- the same edges in every recday and
        both datasets. Rows whose value lies outside the range are NaN-ed so they contribute
        nothing to that block (= the first bin under reference coding) and are counted in
        `CV_results[recday]['bin_occupancy'][name]['n_out_of_range']`; with the range equal to
        `max_transition_seconds` these are 250 ms windows straddling a reward whose block mean
        was pulled up by the previous leg's tail, i.e. leg-START rows, so the first bin is
        where they belong. `scheme='decile'` with `n_bins` is the quantile path with a custom
        bin count. Requires `continuous_basis='onehot'`. Every overridden regressor gets a
        `bin_occupancy` entry (rows per bin, edges in seconds for the time regressors, legs
        reaching each bin) in `CV_results`, printed in the log too.
    ...
    """
    # V3: per-regressor bin placement. Canonicalise the names, derive the column-count
    # overrides, and validate before anything is fitted.
    _OVERRIDABLE = ('speed', 'acceleration', 'time_from_reward', 'time_to_reward',
                    'time_since_A', 'time_to_A', 'distance_from_reward', 'distance_to_reward')
    _TIME_REGS = ('time_from_reward', 'time_to_reward', 'time_since_A', 'time_to_A')
    _bin_ov = {_REGRESSOR_NAME_ALIASES.get(k, k): dict(v)
               for k, v in (binning_overrides or {}).items()}
    _bad_ov = [k for k in _bin_ov if k not in _OVERRIDABLE]
    if _bad_ov:
        raise ValueError(f"binning_overrides for {_bad_ov}: only {_OVERRIDABLE} can be overridden")
    if _bin_ov and continuous_basis != 'onehot':
        raise ValueError("binning_overrides require continuous_basis='onehot'")
    _n_cols_override = {k: int(v['n_bins']) for k, v in _bin_ov.items() if 'n_bins' in v}

    local_groups, local_names = _resolve_regressor_groups(
        regressors_to_include, gp_n_bins=gp_n_bins,
        parameterization=parameterization,
        n_cols_override=_n_cols_override or None,
    )
    _missing_ov = [k for k in _bin_ov if k not in local_names]
    if _missing_ov:
        raise ValueError(f"binning_overrides for {_missing_ov}, which are not in the design "
                         f"{local_names}")
    ...
    for mouse_recday in tqdm(mouse_recdays, desc="Processing recording days"):
        ...
        # ----------------------------------------------------------------
        # Bin edges from all available data (V3: per-regressor `binning_overrides`)
        # ----------------------------------------------------------------
        # The eight continuous arrays, by canonical regressor name and as float. The names
        # below are rebound to the SAME objects, so an in-place NaN written here is what the
        # one-hot encoders see.
        _cont = {'speed': speed_all, 'acceleration': acc_all,
                 'time_from_reward': tf_all, 'time_to_reward': tt_all,
                 'time_since_A': tsa_all, 'time_to_A': tta_all,
                 'distance_from_reward': df_all, 'distance_to_reward': dt_all}
        for _k in _cont:
            _cont[_k] = np.asarray(_cont[_k], dtype=float)
        (speed_all, acc_all, tf_all, tt_all, tsa_all, tta_all, df_all, dt_all) = (
            _cont[k] for k in ('speed', 'acceleration', 'time_from_reward', 'time_to_reward',
                               'time_since_A', 'time_to_A', 'distance_from_reward',
                               'distance_to_reward'))
        _edges_used, _n_out_of_range = {}, {}

        def _edges_for(name):
            arr = _cont[name]
            ov = _bin_ov.get(name)
            if ov is None:
                e = compute_decile_edges(arr, scheme=continuous_binning)
            else:
                sch = ov.get('scheme', 'uniform')
                nb = int(ov.get('n_bins', 10))
                if sch == 'uniform' and ov.get('range_s') is not None:
                    lo_s, hi_s = (float(ov['range_s'][0]), float(ov['range_s'][1]))
                    lo, hi = lo_s * 1000.0 / bin_size_ms, hi_s * 1000.0 / bin_size_ms
                    out = np.isfinite(arr) & ((arr < lo) | (arr > hi))
                    _n_out_of_range[name] = int(out.sum())
                    # Out-of-range rows are 250 ms windows straddling a reward: the block
                    # MEAN of a sawtooth variable mixes the previous leg's tail (large) with
                    # this leg's start (~0). They are leg-start rows. NaN them so
                    # `apply_onehot` zeroes the row -- the first bin under reference coding,
                    # where a leg-start row belongs -- rather than letting `edges[-1] = inf`
                    # drop them into the sparsest, latest bin.
                    arr[out] = np.nan
                    e = compute_decile_edges(arr, n_bins=nb, scheme='uniform',
                                             fixed_range=(lo, hi))
                else:
                    e = compute_decile_edges(arr, n_bins=nb, scheme=sch)
            _edges_used[name] = e
            return e

        speed_edges = _edges_for('speed')
        acc_edges   = _edges_for('acceleration')
        tf_edges    = _edges_for('time_from_reward')
        tt_edges    = _edges_for('time_to_reward')
        tsa_edges   = _edges_for('time_since_A')
        tta_edges   = _edges_for('time_to_A')
        df_edges    = _edges_for('distance_from_reward')
        dt_edges    = _edges_for('distance_to_reward')
        ...
        # ----------------------------------------------------------------
        # V3: bin occupancy for every overridden regressor. Uniform bins over a fixed range
        # are populated by DIFFERENT legs -- only legs longer than t reach the bin at t -- so
        # the late bins are sparse and act as a long-leg indicator; decile bins are equally
        # populated but their edges differ per recday. Both facts are recorded here so the
        # notebook can show them next to the coefficients (GLM_V3.md, caveat 4).
        # ----------------------------------------------------------------
        bin_occupancy = {}
        if _bin_ov:
            _durs = []
            for _s in sessions_for_glm:
                _st = prepared_sessions[_s].get('transition_filter_stats')
                if _st:
                    _durs.extend(t['duration_bins'] for t in _st['per_transition']
                                 if t['pass_time'] and (t['pass_path'] or not filter_correct_paths))
            _durs = np.asarray(_durs, dtype=float)
            for _name, _ov in _bin_ov.items():
                _oh = onehots[_name]
                _e = np.asarray(_edges_used[_name], dtype=float)
                _lower = np.where(np.isfinite(_e[:-1]), _e[:-1], 0.0)   # bin k spans [e[k], e[k+1])
                _rows = _oh.sum(axis=0)
                _is_time = _name in _TIME_REGS
                bin_occupancy[_name] = {
                    'scheme': _ov.get('scheme', 'uniform'),
                    'edges': [float(x) for x in _e],
                    'edges_s': ([float(x) for x in _e * bin_size_ms / 1000.0] if _is_time else None),
                    'rows': [int(x) for x in _rows],
                    'frac_rows': [float(x) for x in _rows / max(float(_rows.sum()), 1.0)],
                    'legs_reaching': [int((_durs > lo).sum()) for lo in _lower],
                    'n_legs': int(len(_durs)),
                    'n_out_of_range': int(_n_out_of_range.get(_name, 0)),
                    'n_rows': int(_oh.shape[0]),
                }
                _occ = bin_occupancy[_name]
                print(f"  bin occupancy — {_name} ({_occ['scheme']}, {len(_rows)} bins, "
                      f"{_occ['n_legs']} legs; {_occ['n_out_of_range']} rows beyond the fixed "
                      f"range coded as bin 0):")
                for k in range(len(_rows)):
                    lo_lab = (f"{_lower[k] * bin_size_ms / 1000.0:6.1f} s" if _is_time
                              else f"{_lower[k]:9.3g}")
                    print(f"    bin {k:2d}  from {lo_lab}   rows {_occ['frac_rows'][k]:6.1%}"
                          f"   legs reaching {_occ['legs_reaching'][k] / max(_occ['n_legs'], 1):6.1%}")
        ...
        if cross_validate:
            ...
            if n_sess < 2:
                ...
                cv_out['rank'] = _rank
                cv_out['engine'] = GLM_VERSION            # V3
                cv_out['bin_occupancy'] = bin_occupancy   # V3: {} unless binning_overrides
                cv_out['n_rows'] = int(X.shape[0])        # V3
...
def compute_tuning_arrays(GLM_results, Permutation_results, regressors_to_include=None,
                          gp_n_bins=10, parameterization='all_bins', n_cols_override=None):
    """
    ...
    Pass the same `regressors_to_include`, `gp_n_bins`, `parameterization` (and, V3, the
    `n_cols_override` implied by `binning_overrides`) used in `run_glm_analysis` so that
    `_beta_direction` indexes the regressor betas with the correct (subset-, bin-count-, and
    coding-specific) index map.
    """
    local_groups, local_names = _resolve_regressor_groups(
        regressors_to_include, gp_n_bins=gp_n_bins,
        parameterization=parameterization,
        n_cols_override=n_cols_override,
    )
    tuned_dict = {}
