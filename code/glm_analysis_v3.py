...
Seeded from `glm_analysis_v2.py`, which is left untouched so the two can be diffed and so
`glm_v3_synthetics.py` control 1 can assert that V3 under its defaults reproduces V2 to
`max|diff| = 0`. Every production fit on disk (`all_regressors__*_250ms_decile`) was made with
V2 and stays reproducible from it. See `GLM_V3.md` for the design this version exists for.
...
Nothing else differs: `diff glm_analysis_v2.py glm_analysis_v3.py` is the change list.
