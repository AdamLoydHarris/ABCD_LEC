...
def preprocess_ephys_session(
    ...
    raw_rec, preprocessed_path = get_probe_recordings(
        subject_ID, config_name, datetime, ephys_path, spikesort_path=spikesort_path
    )
...
def pre_kilosort_processing(
    subject_ID, config_name, raw_rec, preprocessed_path, probe_label=None, IBL_preprocessing=True, plot=True
):
    ...
    if probe_label is None:
        ...
        config_params_path = SPIKESORTING_PATH / "probe_params" / subject_ID / probe_label / config_name
...
def save_channel_assignment_params(
    ...
    subject_paths_df = ephys_paths_df[ephys_paths_df["subject_ID"] == subject_ID]
    config_names = subject_paths_df.config_name.unique()
    if probe_label is None:
        base_path = SPIKESORTING_PATH / "probe_params" / subject_ID
    else:
        base_path = SPIKESORTING_PATH / "probe_params" / subject_ID / probe_label
    for cn in config_names:
        print(f"processing {cn} for {subject_ID} with")
        assignment_path = base_path / cn
        assignment_path.mkdir(parents=True, exist_ok=True)
        _path_df = subject_paths_df.set_index("config_name")
        config_info = _path_df.loc[cn]
        raw_rec, preprocessed_path = get_probe_recordings(
            config_info.subject_ID,
            cn,
            config_info.datetime.isoformat(),
            config_info.ephys_path,
...
def get_probe_recordings(
    subject_ID, config_name, datetime, ephys_path, probe_label=None, spikesort_path=SPIKESORTING_PATH
):
    """ """
    if probe_label is None:
        ...
        spikesorting_path = Path(spikesort_path) / subject_ID / probe_label / config_name / datetime
        raw_rec = se.read_openephys(ephys_path, stream_id=probe_label)
...
def save_probe_config_params(
    ...
    INPUT EXAMPLES:
        ...
        probe_label = 'ProbeA' #NB: for multi-probes only, as probe info is saved in subject subfolders
    ...
    if probe_label is None:
        ...
        base_path = SPIKESORTING_PATH / "probe_params" / subject_ID / probe_label
    for cn in config_names:
        print(f"processing {cn} for {subject_ID}")
        probe_params_path = base_path / cn
        probe_params_path.mkdir(parents=True, exist_ok=True)
        config_info = _path_df.loc[cn]
        raw_rec = se.read_openephys(config_info.ephys_path, stream_id="0")
...
def get_ephys_paths_df():
    ...
    if (EPHYS_PATH / "ephys_paths_df.tsv").exists():  # load the df if it already exists.
        ...
        ephys_paths_df["datetime"] = ephys_paths_df["datetime"].apply(
            lambda x: pd.to_datetime(x)
        )  # change datetime from str to object
        # update completion
        completion = []
        for path in all_ephys_paths:
            path = Path(path)
            subject_ID = path.parts[-3]
            config_name = path.parts[-2]
            datetime_string = path.parts[-1]
            dt = datetime.strptime(datetime_string, "%Y-%m-%d_%H-%M-%S")
            spikesorted_path = SPIKESORTING_PATH / subject_ID / config_name / dt.isoformat()
            ## For multiple probes we need to check if both probes have been sorted for that session:
            if spikesorted_path.exists():
                spike_sorting_completed = (spikesorted_path / "DONE.txt").exists()
                completion.append(spike_sorting_completed)
            else:
                completion.append(False)  # if no spikesorting path exists, then it's not completed.
        try:  # this will fail if more data has been uploaded:
            ephys_paths_df["spike_sorting_completed"] = completion
        except:
            print(
                f"More data ({len(completion)} files) has been uploaded since last dataframe generation ({len(ephys_paths_df)} files).\n Generating ephys_paths_df from scratch."
            )
    ...
    if not (EPHYS_PATH / "ephys_paths_df.tsv").exists():
        ...
        for path in all_ephys_paths:
            ...
            spike_sorting_completed = (
                SPIKESORTING_PATH / subject_ID / config_name / dt.isoformat() / "DONE.txt"
            ).exists()
            try:
