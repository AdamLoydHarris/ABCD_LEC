"""LEC vs PFC comparison for the V5 anchoring regression.

Reads the exported `.npz` files that `elasticnet_regression_v5.export_regression_outputs`
...
dataset per lag direction and it does the rest.
    ...
    python code/elasticnet_v5_compare.py                       # newest run of each
    python code/elasticnet_v5_compare.py --lec-root data/figures \\
...
def resolve_latest(root, direction, estimator=None, tag=None):
    """Newest `{estimator}[_{tag}]_v5_{direction}_*` directory under `root`.

    `estimator` is 'poisson' / 'elasticnet' / 'linear', or None for whichever is newest.
    Also matches the legacy `elasticnet_v5_*` layout, which was written under that name even
    for Poisson runs -- check `run_config.json` if an old folder looks surprising.

    `tag` selects a variant written by `run_tag`, currently only 'zscore' (a recday-z-scored fit
    target). With `tag=None` the tagged directories are EXCLUDED rather than merely unmatched:
    the estimator wildcard would otherwise let `elasticnet_zscore_v5_past_*` answer a request
    for the newest raw-count run, silently comparing a z-scored run against a reproduction.
    """
    pattern = f'{estimator or "*"}{"_" + tag if tag else ""}_v5_{direction}_*'
    hits = sorted(glob.glob(os.path.join(root, pattern)))
    hits = [h for h in hits if os.path.isdir(h)]
    if tag is None:
        hits = [h for h in hits if '_zscore_v5_' not in os.path.basename(h)]
    if not hits:
        raise FileNotFoundError(f'no {pattern} directory under {root}'
                                + (' (excluding _zscore variants)' if tag is None else ''))
    return hits[-1]
...
def check_comparable(dirs, keys=('estimator', 'num_lags', 'alpha_mode', 'elasticnet_alpha',
                                 ...
                                 'nonzero_lag_min', 'nonzero_lag_max', 'y_scaling')):
...
def per_recday_stats(runs, min_neurons=2):
    ...
    for recday, res in sorted(runs.items()):
        ...
        rows.append({
            ...
            # non-zero-lag neurons are scored with the REDUCED-beta prediction (v5); the
            # all-betas r is kept alongside for reference
            'mean_r': (float(np.nanmean(res['mean_corrs_nonzero'][sel]))
                       if sel.any() else np.nan),
            'mean_r_all_betas': float(np.nanmean(res['mean_corrs'][sel])) if sel.any() else np.nan,
...
def main():
    ...
    ap.add_argument('--tag', default=None, choices=['zscore'],
                    help='compare the tagged run variant instead of the raw-count runs '
                         '(zscore = recday-z-scored fit target)')
    ap.add_argument('--out', default=None,
                    help='figure stem; defaults to figures/{estimator}[_{tag}]_v5_lec_vs_pfc')
    args = ap.parse_args()
    out = args.out or os.path.join(
        FIG_DIR, f'{args.estimator or "any"}{"_" + args.tag if args.tag else ""}_v5_lec_vs_pfc')

    dirs = {}
    for name, root in (('LEC', args.lec_root), ('PFC', args.pfc_root)):
        try:
            dirs[name] = {d: resolve_latest(root, d, args.estimator, tag=args.tag)
